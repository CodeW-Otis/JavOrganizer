...
public abstract partial class SiteScraper : IDisposable
{
    ...
    // A ban page is recognised by an explicit statement that *this address*
    // was blocked. Bare phrases like "access denied" or "too many requests"
    // are deliberately NOT matched on their own: sites embed those in
    // JavaScript localisation tables and unrelated copy (jav.guru ships
    // "Too many requests. Please slow down." for its comment widget), and
    // treating that as a ban disabled the site for six hours on every scrape.
    [GeneratedRegex(@"banned your access|has banned|your (?:ip|address) has been (?:blocked|banned)|禁止了你的訪問|blocked your access|access to this (?:page|site) has been (?:denied|blocked|banned)|(?:ip|address) (?:is|has been) (?:blocked|banned)", RegexOptions.IgnoreCase)]
    ...
    private static readonly Regex BannedRegexInstance = new(@"banned your access|has banned|your (?:ip|address) has been (?:blocked|banned)|禁止了你的訪問|blocked your access|access to this (?:page|site) has been (?:denied|blocked|banned)|(?:ip|address) (?:is|has been) (?:blocked|banned)", RegexOptions.IgnoreCase | RegexOptions.Compiled);
    ...
    /// <summary>
    /// Reports whether a scraped page is an explicit "this address was
    /// banned" notice. Exposed so the offline suite can pin the exact
    /// accept/reject boundary: a false positive disables a site for six
    /// hours, and a false negative means the plugin keeps hammering a ban.
    /// </summary>
    /// <param name="html">Page HTML.</param>
    /// <returns><c>true</c> when the page states this address is banned.</returns>
    internal static bool IsBanPage(string html)
        => !string.IsNullOrEmpty(html) && BannedRegex().IsMatch(html);
